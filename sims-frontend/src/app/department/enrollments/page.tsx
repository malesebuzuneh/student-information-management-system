...
import { Enrollment, Notification, TableColumn } from '@/types';
...
const EnrollmentsPage: React.FC = () => {
  ...
  const showNotification = (message: string, type: Notification['type'] = 'success'): void => {
  ...
  const fetchEnrollments = async (): Promise<void> => {
    try {
      setLoading(true);
      const response = await api.get<{ pending_enrollments: Enrollment[] }>('/department/pending-enrollments');
  ...
  const handleApproval = async (courseId: number, studentId: number, action: 'approve' | 'reject'): Promise<void> => {
    try {
      ...
      const response = await api.put<{ message: string }>(endpoint);
  ...
  const getStatusColor = (status: Enrollment['status']): string => {
    switch (status) {
  ...
  const getStatusIcon = (status: Enrollment['status']): React.ReactNode => {
    switch (status) {
  ...
  const filteredEnrollments = enrollments.filter((enrollment: Enrollment) => {
    const matchesSearch = 
      ...
      enrollment.student_id?.toString().toLowerCase().includes(searchTerm.toLowerCase()) ||
